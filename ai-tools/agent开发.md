...
**目标**：在 PR / commit 的明确差异范围内，发现本次改动引入的正确性、安全、可靠性和兼容性问题；每条结论必须能定位代码行、说明触发条件并给出证据，而不是泛泛点评代码风格。

```text
base diff + 仓库规则 + 相关调用链 / 测试
  -> 编译 / 测试 / lint / SAST 等确定性检查
  -> LLM 做跨文件语义推理
  -> Evidence Gate（行号、触发条件、影响、与 diff 的因果）
  -> 去重、严重级校准、只读 Finding
  -> 人工接受/拒绝标签 -> Eval 集与规则迭代
```

**推荐架构**：确定性工具保底，受控 Context 补齐变更附近的接口、调用方和测试，再由 LLM 做难以规则化的语义推理；默认只读审查，修复应是独立、可验证的下一步。重点指标不是“报了多少问题”，而是 P0/P1 缺陷召回率、人工确认率、误报密度、行定位/证据准确率、严重级校准、p95 审查时延和每 PR 成本。

完整的场景题、评测集构建和指标公式见 [Agent_场景题_面经合集.md](Agent_场景题_面经合集.md)。
...
| Code Review | diff Workflow + 语义推理 | base diff、仓库规则、调用链与测试 | 只读、证据门、严重级校准、人工确认 |
